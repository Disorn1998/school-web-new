...
const LandingPage = () => {
  ...
  const topUtilityLinks = ["Owls", "Alumni", "Careers", "Give", "Contact"];
  const mainNavLinks = ["About Us", "Admission", "Academics", "Arts", "Athletics", "Student Life"];
    ...
    <div className="w-full min-h-screen font-sans bg-white text-slate-800">
      ...
      <nav className={`w-full z-40 transition-all duration-300 ${isScrolled ? 'fixed top-0 bg-white shadow-md py-4' : 'absolute top-8 bg-transparent py-6'} px-8 flex justify-between items-center`}>
        <div className="flex items-center gap-2 cursor-pointer" onClick={() => window.scrollTo(0, 0)}>
          {/* Logo Placeholder */}
          <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-xl ${isScrolled ? 'bg-[#00523e] text-white' : 'bg-white text-[#00523e]'}`}>
            O
          </div>
          <span className={`text-2xl font-black tracking-tight uppercase ${isScrolled ? 'text-[#00523e]' : 'text-white drop-shadow-md'}`}>
            Overlake
          </span>
        </div>
        ...
        <div className="hidden lg:flex items-center gap-8">
          {mainNavLinks.map(link => (
            <span key={link} className={`font-bold uppercase tracking-wider text-[13px] flex items-center gap-1 cursor-pointer transition-colors ${isScrolled ? 'text-[#00523e] hover:text-[#f2a900]' : 'text-white drop-shadow-md hover:text-[#f2a900]'}`}>
              {link} <ChevronDown size={14} className="opacity-50" />
            </span>
          ))}
          <button onClick={() => navigate('/apply')} className="bg-[#f2a900] text-white px-6 py-2.5 font-bold uppercase tracking-wider text-[13px] hover:bg-[#d89600] transition-colors shadow-md">
            Inquire
          </button>
        </div>
      ...
      {/* Mobile Menu Overlay */}
      {mobileMenuOpen && (
        <div className="fixed inset-0 bg-white z-40 flex flex-col pt-24 px-8 pb-8">
          <div className="flex flex-col gap-6 text-[#00523e] font-black text-2xl uppercase tracking-wider">
            {mainNavLinks.map(link => (
              <span key={link} className="border-b border-gray-100 pb-2">{link}</span>
            ))}
      ...
      <div className="w-full bg-[#f4f4f4] py-16 px-8 flex justify-center -mt-10 relative z-30">
        <div className="max-w-6xl w-full grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className="bg-white p-8 shadow-xl border-t-4 border-[#00523e] flex flex-col items-start hover:-translate-y-2 transition-transform cursor-pointer group">
            ...
            <button onClick={() => navigate('/login')} className="font-bold text-[#00523e] uppercase text-sm tracking-wider flex items-center gap-2 group-hover:text-[#f2a900] transition-colors">
              Login Now <span className="text-xl"></span>
            </button>
          </div>

          <div className="bg-white p-8 shadow-xl border-t-4 border-[#f2a900] flex flex-col items-start hover:-translate-y-2 transition-transform cursor-pointer group">
            ...
            <p className="text-gray-600 mb-6 flex-1">Join our community. Learn about the application process, visit our campus, and discover Overlake.</p>
            <button onClick={() => navigate('/apply')} className="font-bold text-[#00523e] uppercase text-sm tracking-wider flex items-center gap-2 group-hover:text-[#f2a900] transition-colors">
          ...
          <div className="bg-white p-8 shadow-xl border-t-4 border-[#00523e] flex flex-col items-start hover:-translate-y-2 transition-transform cursor-pointer group">
            ...
            <button onClick={() => navigate('/admin/login')} className="font-bold text-[#00523e] uppercase text-sm tracking-wider flex items-center gap-2 group-hover:text-[#f2a900] transition-colors">
      ...
      {/* Overlake Stats / Highlight */}
      <div className="w-full bg-[#00523e] text-white py-24 px-8 text-center">
        <h2 className="text-4xl md:text-5xl font-black uppercase tracking-tight mb-16">The Overlake Difference</h2>
      ...
      <footer className="w-full bg-[#1a1a1a] text-white py-12 px-8">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 bg-[#00523e] text-white rounded-full flex items-center justify-center font-bold text-2xl">O</div>
            <div>
